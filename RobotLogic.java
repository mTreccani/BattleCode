...
public abstract class RobotLogic {
	 ...
	 */
	 ...
	 public void tankSquadStrategy() throws GameActionException	 {
		 ...
		 boolean tankSquad = rc.readBroadcast(SQUAD_0)!=0 && rc.readBroadcast(SQUAD_1)!=0 && rc.readBroadcast(SQUAD_2)!=0 && rc.readBroadcast(SQUAD_3)!=0;
		 if(tankSquad && soldierSquad>=3 && !nearEnemyArchon){
			 if(!rc.hasMoved()) tryMove (directionToEnemyArchon());
		 }
		 else{
			 if(!rc.hasMoved()) tryMove (randomDirection());
